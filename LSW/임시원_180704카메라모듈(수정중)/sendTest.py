...
class MyDelegate(btle.DefaultDelegate):
    ...
    def sendValuesUsingThread(self):  # 온도정보를 실시간으로 전송
        ...
        t2 = threading.Thread(target=self.sendValues)
        t2.daemon = True
        t2.start()
    ...
    def sendValues(self):
        ...
        blueTooth = btle.Peripheral('3C:A3:08:9F:D4:25')
        blueTooth.setDelegate(MyDelegate(0))
        while (self.stopValue != str(1)):
            if blueTooth.waitForNotifications(1.0):
                continue
            print("waiting...")
